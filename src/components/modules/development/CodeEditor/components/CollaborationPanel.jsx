...
const CollaborationPanel = ({ onClose }) => {
  ...
  const handleInvite = () => {
    ...
    setConnectionStatus(`Enviando invitación a ${inviteEmail}...`);
  ...
  const handleClearAll = () => {
    if (confirm('¿Expulsar a todos los colaboradores?')) {
      clearAllCollaborators();
      setConnectionStatus('Todos los colaboradores expulsados');
      setTimeout(() => setConnectionStatus('Conectado'), 2000);
    }
  };

  const getUserColor = (userId) => {
    const colors = ['#13ecc8', '#f59e0b', '#8b5cf6', '#ec4899', '#3b82f6', '#10b981'];
